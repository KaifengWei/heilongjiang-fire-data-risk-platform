...
# Add task-scoped daily/map-friendly context before the processing check inside
# render_task_detail().  The generic text ``has_completed_processing = (`` also
# appears inside build_analysis_summary(), so anchor it to the immediately
# preceding task_region_statistics block instead of counting the short token.
if "task_daily_series = (" not in app:
    anchor = '''        task_region_statistics = (\n            statistics_service\n            .task_region_statistics(\n                task_id\n            )\n        )\n\n        has_completed_processing = (\n'''
    insertion = '''        task_region_statistics = (\n            statistics_service\n            .task_region_statistics(\n                task_id\n            )\n        )\n\n        task_daily_series = (\n            statistics_service\n            .task_daily_series(task_id)\n        )\n\n        analysis_summary = (\n            build_analysis_summary(task)\n        )\n\n        region_feature_collection = (\n            database.region_feature_collection()\n        )\n\n        task_region_ranking = sorted(\n            task_region_statistics,\n            key=lambda row: (\n                int(row.get("active_fire_count", 0)),\n                float(row.get("burned_area_km2", 0.0)),\n            ),\n            reverse=True,\n        )\n\n        has_completed_processing = (\n'''
